        ...
        public async Task<MessageModel<string>> AddScheduleJobAsync(TasksQz tasksQz)
        {
            ...
            try
            {
                if (tasksQz != null)
                {
                    ...
                    DateTimeOffset startRunTime = DateBuilder.NextGivenMinuteDate(tasksQz.BeginTime, 1);//设置开始时间
                    if (tasksQz.EndTime == null)
                    {
                        tasksQz.EndTime = DateTime.MaxValue.AddDays(-1);
                    }
                    DateTimeOffset endTunTime = DateBuilder.NextGivenSecondDate(tasksQz.EndTime, 1);//设置暂停时间
                    #endregion
